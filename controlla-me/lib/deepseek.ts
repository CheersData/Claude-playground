/**
 * DeepSeek Client — wrapper per DeepSeek V3 e DeepSeek R1.
 *
 * Usa l'SDK OpenAI con baseURL custom (API compatibile OpenAI).
 *
 * ⚠️  ATTENZIONE PRIVACY: i server DeepSeek sono in Cina.
 * NON inviare documenti legali sensibili senza consenso esplicito dell'utente.
 * Consigliato solo per task non-sensibili (classificazione, riformulazione).
 *
 * Richiede: DEEPSEEK_API_KEY nel .env.local
 * Se la chiave non è presente, isDeepSeekEnabled() ritorna false.
 */

import OpenAI from "openai";

export { parseAgentJSON } from "./anthropic";

// ─── Singleton ───

let _client: OpenAI | null = null;

function getDeepSeekClient(): OpenAI {
  if (!_client) {
    if (!process.env.DEEPSEEK_API_KEY) {
      throw new Error("Missing DEEPSEEK_API_KEY environment variable");
    }
    _client = new OpenAI({
      apiKey: process.env.DEEPSEEK_API_KEY,
      baseURL: "https://api.deepseek.com",
    });
  }
  return _client;
}

// ─── Models ───

/** DeepSeek V3.2 (chat): general purpose ($0.28/$0.42 per 1M token, cache hit $0.028) */
export const DEEPSEEK_MODEL_V3 = "deepseek-chat";
/** DeepSeek R1 (reasoner): reasoning ($0.55/$2.19 per 1M token) — attenzione: reasoning token nascosti */
export const DEEPSEEK_MODEL_R1 = "deepseek-reasoner";

const MAX_RETRIES = 3;
const RETRY_WAIT_MS = 15_000;

// ─── Public API ───

/** Check if DeepSeek is available (API key present). */
export function isDeepSeekEnabled(): boolean {
  return !!process.env.DEEPSEEK_API_KEY;
}

export interface DeepSeekGenerateConfig {
  /** Model to use. Default deepseek-chat (V3). */
  model?: string;
  /** System prompt. */
  systemPrompt?: string;
  /** Max output tokens. Default 4096. */
  maxTokens?: number;
  /** Temperature. Default 0.2. */
  temperature?: number;
  /** Force JSON output via response_format. Default true. */
  jsonOutput?: boolean;
  /** Agent name for logging. Default "DEEPSEEK". */
  agentName?: string;
}

export interface DeepSeekResult {
  text: string;
  usage: { inputTokens: number; outputTokens: number };
  durationMs: number;
}

/**
 * Generate a response with a DeepSeek model.
 * Includes retry logic (3 attempts, 15s wait on 429).
 *
 * ⚠️  PRIVACY: i dati transitano su server in Cina.
 */
export async function generateWithDeepSeek(
  prompt: string,
  config: DeepSeekGenerateConfig = {}
): Promise<DeepSeekResult> {
  const {
    model = DEEPSEEK_MODEL_V3,
    systemPrompt,
    maxTokens = 4096,
    temperature = 0.2,
    jsonOutput = true,
    agentName = "DEEPSEEK",
  } = config;

  const client = getDeepSeekClient();
  const inputChars = prompt.length + (systemPrompt?.length ?? 0);
  const label = agentName.toUpperCase();

  console.log(
    `\n${"=".repeat(60)}\n[API] → ${label} | model: ${model} | max_tokens: ${maxTokens} | input: ~${inputChars} chars`
  );

  const start = Date.now();

  const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [];
  if (systemPrompt) {
    messages.push({ role: "system", content: systemPrompt });
  }
  messages.push({ role: "user", content: prompt });

  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      const response = await client.chat.completions.create({
        model,
        messages,
        max_tokens: maxTokens,
        temperature,
        ...(jsonOutput ? { response_format: { type: "json_object" as const } } : {}),
      });

      const elapsed = Date.now() - start;
      const text = response.choices[0]?.message?.content ?? "";
      const inputTokens = response.usage?.prompt_tokens ?? 0;
      const outputTokens = response.usage?.completion_tokens ?? 0;

      console.log(
        `[API] ← ${label} | ${(elapsed / 1000).toFixed(1)}s | tokens: ${inputTokens} in / ${outputTokens} out | stop: ${response.choices[0]?.finish_reason} | risposta totale: ${text.length} chars`
      );
      console.log(
        `[API]   Risposta (primi 8000 char):\n${text.slice(0, 8000)}${text.length > 8000 ? "\n... [troncato, altri " + (text.length - 8000) + " chars]" : ""}`
      );
      console.log("=".repeat(60));

      return {
        text,
        usage: { inputTokens, outputTokens },
        durationMs: elapsed,
      };
    } catch (err: unknown) {
      const status = (err as { status?: number }).status;
      const isRateLimit =
        status === 429 ||
        (err instanceof Error && err.message.includes("rate_limit"));

      if (isRateLimit && attempt < MAX_RETRIES) {
        console.log(
          `[API] ⏳ ${label} rate limit! Attendo ${RETRY_WAIT_MS / 1000}s (tentativo ${attempt + 1}/${MAX_RETRIES})...`
        );
        await new Promise((r) => setTimeout(r, RETRY_WAIT_MS));
        continue;
      }

      throw err;
    }
  }

  throw new Error(`${label}: max retries exceeded`);
}

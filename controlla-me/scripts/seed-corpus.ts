...
interface ArticleRow {
  ...
  url: string | null;
  in_force: boolean;
  // Backward compat with old schema (law_source/article_reference NOT NULL)
  law_source: string;
...
async function getExistingCount(sourceName: string): Promise<number> {
  // Try new column first, fall back to old
  let { count, error } = await supabase
    .from("legal_articles")
    .select("*", { count: "exact", head: true })
    .eq("law_source", sourceName);

  if (error) {
    // Fallback: try source_id (new schema)
    const res = await supabase
      .from("legal_articles")
      .select("*", { count: "exact", head: true })
      .eq("source_id", sourceName.toLowerCase().replace(/\s+/g, "_"));
    if (res.error) {
      console.warn(`  WARN: Errore conteggio per ${sourceName}:`, res.error.message);
      return 0;
    }
    return res.count || 0;
  }
...
async function shouldSkipSource(source: CorpusSource): Promise<boolean> {
  const existing = await getExistingCount(source.name);
...
async function main() {
  ...
  for (const source of ALL_SOURCES) {
    const count = await getExistingCount(source.name);
    if (count > 0) {

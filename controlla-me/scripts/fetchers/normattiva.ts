/**
 * Fetcher Normattiva — Scarica leggi italiane da normattiva.it.
 *
 * Normattiva NON ha un'API REST pubblica documentata.
 * Opzioni disponibili:
 * 1. HTML scraping della pagina "testo vigente" (usato qui)
 * 2. URN resolver (restituisce HTML, stesso parsing)
 * 3. Endpoint XML non documentato (instabile)
 *
 * Strategia:
 ...
 *
 * NOTA: Normattiva può bloccare scraping aggressivo.
 * Se il fetch fallisce, lo script logga un warning e procede.
 * Il Codice Civile usa HuggingFace (più affidabile).
 */
...
export async function fetchNormattiva(source: NormattivaSource): Promise<LegalArticle[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`  [NORM] HTTP ${response.status} — provo paginazione...`);
    ...
    if (articles.length === 0) {
      console.warn(`  [NORM] Nessun articolo dal parsing. Provo paginazione...`);
      return fetchMultiPage(source);
    }
    ...
    if (articles.length < source.expectedArticles * 0.5 && source.expectedArticles > 50) {
      console.log(`  [NORM] Solo ${articles.length}/${source.expectedArticles} — provo paginazione...`);
...
async function fetchMultiPage(source: NormattivaSource): Promise<LegalArticle[]> {
  console.log(`  [NORM] Paginazione per ${source.name}...`);
  ...
  while (page <= maxPages && consecutiveEmpty < 3) {
    try {
      ...
      if (newArticles.length === 0) {
        ...
        if (page % 10 === 0) {
          console.log(`  [NORM] Pagina ${page}: ${allArticles.length} articoli`);
        }
      }
    } catch {
      consecutiveEmpty++;
  ...
  console.log(`  [NORM] Paginazione: ${allArticles.length} articoli in ${page - 1} pagine`);
...
function logValidation(count: number, source: NormattivaSource): void {
  ...
  if (pct < source.minThresholdPct) {
    console.warn(
      `  [NORM] ATTENZIONE: ${count} articoli (${pct.toFixed(0)}% del previsto ${source.expectedArticles})`
    );
  } else {
    console.log(
      `  [NORM] ${count} articoli (${pct.toFixed(0)}% del previsto ${source.expectedArticles})`
    );
  }
}

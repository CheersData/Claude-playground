/**
 * scripts/lib/llm.ts — Helper LLM per script interni (ZERO COSTI)
 *
 ...
 *
 * Sostituisce tutti gli usi di `claude -p` / `spawnSync("claude")` negli script.
 * Pattern: stessa interfaccia di generate() ma con retry chain hardcoded su free tier.
 */

import { generate } from "@/lib/ai-sdk/generate";
import type { GenerateConfig, GenerateResult } from "@/lib/ai-sdk/types";
...
export async function callLLMFull(
  ...
  if (model) {
    ...
    console.log(
      `  [LLM] ✓ ${result.provider}/${result.model} | ${result.durationMs}ms | ${result.usage.inputTokens}in/${result.usage.outputTokens}out`
    );
    return result;
  }
  ...
  for (const { key, provider } of FREE_CHAIN) {
    ...
    try {
      ...
      console.error(`  [LLM] ✗ ${key}: ${msg.slice(0, 120)}`);
      errors.push(`${key}: ${msg}`);
    }
  }
